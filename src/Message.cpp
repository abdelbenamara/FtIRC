...
/*   Updated: 2024/11/02 10:01:30 by abenamar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "Message.hpp"

std::locale const irc::Message::LOCALE;

std::string const irc::Message::CRLF = "\r\n";

std::size_t const irc::Message::MAX_LEN = MSG_LINE_LEN, irc::Message::MAX_CHARS = irc::Message::MAX_LEN - irc::Message::CRLF.length(), irc::Message::NUM_RPL_LEN = 3, irc::Message::PARAMS_MAX_LEN = 15;

bool irc::Message::Builder::isNotInCommandFormat(char const &c) { return (!std::isalpha(c, irc::Message::LOCALE)); }
...
irc::Message::Builder &irc::Message::Builder::withCommand(std::string const &command)
{
    try
    {
        ...
        else if (std::isdigit(command.at(0), irc::Message::LOCALE))
        {
            if (command.length() != irc::Message::NUM_RPL_LEN)
                throw std::length_error("std::length_error: " + utils::to_string(command.length()) + ": numeric reply command must have exactly " + utils::to_string(irc::Message::NUM_RPL_LEN) + " characters");
            else if (!std::isdigit(command.at(1), irc::Message::LOCALE) || !std::isdigit(command.at(2), irc::Message::LOCALE))
                throw std::domain_error("std::domain_error: numeric reply command must have only digit characters");
        }
        else if (std::find_if(command.begin(), command.end(), irc::Message::Builder::isNotInCommandFormat) != command.end())
...
irc::Message::Builder &irc::Message::Builder::addParameter(std::string const &parameter)
{
    try
    {
        if (this->parameters.size() == irc::Message::PARAMS_MAX_LEN)
            throw std::length_error("std::length_error: a message must not have more than " + utils::to_string(irc::Message::PARAMS_MAX_LEN) + " parameters");
...
irc::Message::Builder &irc::Message::Builder::withParameters(std::vector<std::string> const &parameters)
{
    try
    {
        if (parameters.size() > irc::Message::PARAMS_MAX_LEN)
            throw std::length_error("std::length_error: " + utils::to_string(parameters.size()) + ": a message must not have more than " + utils::to_string(irc::Message::PARAMS_MAX_LEN) + " parameters");
...
{
    static std::string const sep = ' ' + irc::Message::CRLF;
    std::size_t cur, max, pos;
    irc::Message::Builder builder;

    try
    {
        if (input.empty() || input.length() > irc::Message::MAX_LEN)
            throw std::length_error("std::length_error: " + utils::to_string(input.length()) + ": input must not be empty or have more than " + utils::to_string(irc::Message::MAX_LEN) + " characters");
        else if (input.find('\0') != std::string::npos)
            throw std::domain_error("std::domain_error: input must not contain a NUL character");
        else if (input.compare(input.length() - irc::Message::CRLF.length(), std::string::npos, irc::Message::CRLF))
            throw std::invalid_argument("std::invalid_argument: input must be terminated with a CRLF pair");
        else if (input.find_first_of(irc::Message::CRLF) < input.length() - irc::Message::CRLF.length())
        ...
        max = input.length() - irc::Message::CRLF.length();
